/*
Utils helper methods tunned from https://github.com/nicejade/vuepress-web-app/blob/master/docs/helper/utils.js
Thanks to @nicejade
*/
  ...
  findSyncFolderName (startPath) {
    ...
    function finder(path) {
      ...
      files.forEach((fileName, index) => {
        console.debug(fileName, index)
        const fPath = join(path, fileName)
        const stats = fs.statSync(fPath)
        if (stats.isDirectory() && fileName != "hidden") finder(fPath)
